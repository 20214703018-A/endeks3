...
| `ulasim_ve_hareketlilik_gps.sqlite` | `arac_gps_koridor_profili`<br>`arac_gps_saatlik_akis`<br>`yayalastirilmis_ticari_yollar`<br>`yaya_sayim_sensor_istasyonlari`<br>`istasyon_yaya_tahliye_hacmi` | 2.456 arter profili<br>15.000 saatlik akış<br>5.787 yaya aksı<br>2.562 sensör ölçümü<br>14.504 istasyon debisi | İstanbul, İzmir, Antalya, Bursa, Muğla | 1M Araç Floating Car Data, sensör bazlı yaya sayımları, metro tahliye debileri |
| `bddk_finturk_finansal_gostergeler.sqlite` | `bddk_il_kredi_ve_mevduat`<br>`bddk_il_bireysel_finans`<br>`bddk_il_sektorel_krediler`<br>`bddk_il_sube_ve_likidite` | 243 satır/tablo<br>(3 Çeyrek x 81 İl) | 81 İl, 973 İlçe Bankacılık Ağı | Resmî BDDK FİNTÜRK kredi, mevduat, bireysel kredi kartı ve sektörel krediler |
...
| `google_places_ve_yogunluk.sqlite` | `google_places_ticari_yogunluk` | 83 çıpa koridor<br>350K+ gerçek yorum | Batı Büyükşehirleri & Ankara | Google Maps tersine mühendislik, müşteri hacmi, puan, çalışma saatleri |
| `yemek_ve_market_teslimat_ekosistemi.sqlite` | `teslimat_depolari_darkstore`<br>`uye_restoranlar_ve_hacim` | 102 darkstore/hub<br>37 üye restoran | Türkiye Geneli & Büyükşehirler | Yemeksepeti, Getir, Trendyol Go, Migros Hemen depoları ve restoran sipariş hacimleri |
